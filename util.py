...
# 计算注意图之间的边界相似度
def cal_sim(images, base_index = 0):    # 选一个 base_index 作为“参照 token”，计算 其它 token 与它的 attention 相似度
    base = np.array(images[base_index]).flatten()   # 选基线token
    image = images
    image.pop(base_index)   # 把基线token从比较列表中移除
    scros = []
    max_scros = 0   # 初始化变量
    for i, im in enumerate(image):  # 逐 token 计算 attention 相似度
        im = np.array(im).flatten()
        sims = cosine_similarity(im.reshape(1, -1), base.reshape(1, -1))
        scros.append(float(sims[0]))    # 保存相似度
        if sims > max_scros:    # 找“最像 base 的 token”
            max_scros = sims
            max_index = i + 1
    scros.insert(base_index, 1.0)   # 把 base 自己的相似度补回去
